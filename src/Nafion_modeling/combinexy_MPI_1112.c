...
#define psize 4 /*number of C particle*/
#define delta 2.0	

char file1[BUF]="withwaterlayer.dump";
...
double rmax[2]={160.0,170.0};
double rmin[2]={110.0,110.0};
...
int  read_dump (char* file, int option[ATOM][2], double array[ATOM][6], int mask[ATOM]);
void read_config();
void write_check (int option[ATOM][2], double array[ATOM][6]);
void write_inputrd (int option[ATOM][2], int tot, double array[ATOM][6], double center[psize][3], int mask[ATOM]);
...
int main(int argc,char **argv)
{
	...
	double	pi,x,y,z,r,rv,tmass,phi,theta,rr,r1,r2;
	double  vx,vy,vz;
	...
	int *del_F, *del_FF, modify,vd[3], *msk;
...
//        srand(16);
    ...
    total1=read_dump(file1,atom1c,pos1v,msk);
	for(i = 0; i < 3;i++){axis1[i]=laxis[i];}
    total2=read_dump(file2,atom2c,pos2v,msk);
...
//				printf("%d\n",a);
	...
	if( fp4==NULL ){
		if (pid==0) printf("Warnning : These is no center.txt \n");
		panum=1;
	...
	xx=axis2[0]*(rand()%1000)/1000.0+axis2[0];
	yy=axis2[1]*(rand()%1000)/1000.0+axis2[1];
...
//				if ((x0>0)&&(x0<axis1[0]-delta)&&(y0>0)&&(y0<axis1[1]-delta)&&(z0>0)&&(z0<axis1[2]-delta)) flag=1;
...
// hantei
				for(j = 0; j < panum; j++) {
					...
					dz=z0-paori[j][2];
					r=dx*dx+dy*dy+dz*dz;
					if (r >= rmin[k]*rmin[k]) flag1+=1;
					if (r <= rmax[k]*rmax[k]) flag2+=1;
					if (r < rr) pa=j;
                    if (r < rr) rr=r;
				}
			...
			if (ia < molecules[k][1]){
				...
				for(j = 0; j <molecules[k][0];j++){
					...
					for (s = 0; s <total1;s++){
						...
						r=dx*dx+dy*dy+dz*dz;
						dz=pos3v[a][2]-pos1v[s][2]+axis[2];
						r1=dx*dx+dy*dy+dz*dz;
						dz=pos3v[a][2]-pos1v[s][2]-axis[2];
						r2=dx*dx+dy*dy+dz*dz;
					if (r <rv*rv){del_F[e]=0; break;}
					if (r1 <rv*rv){del_F[e]=0; break;}
					if (r2 <rv*rv){del_F[e]=0; break;}
					}
					if (r <rv*rv){del_F[e]=0; break;}
					if (r1 <rv*rv){del_F[e]=0; break;}
					if (r2 <rv*rv){del_F[e]=0; break;}
...
//					printf("%d %.3lf %.3lf %.3lf \n",atom0c[c][0],pos0v[c][0],pos0v[c][1],pos0v[c][2]);
	...
	if (pid==0) minimum(pos0v, 0.0);
    if (pid==0) write_center(center);
	if (pid==0) write_inputrd(atom0c,total1,pos0v,center,msk);
...
int read_dump (char *file, int option[ATOM][2], double arr[ATOM][6], int mask[ATOM]) {
	int i, j, k, a, b, c,m;
...
/*        sscanf( buf, "%d%d%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf",
    	...
    	sscanf( buf, "%d%d%lf%lf%lf%lf%lf%lf%d", &c,&b,&x,&y,&z,&vx,&vy,&vz,&m);
        option[c-1][0]=b;
        option[c-1][1]=b;
        arr[c-1][0]=x-daxis[0];
        arr[c-1][1]=y-daxis[1];
...
//    	if (option[i][1]<10){
    		a++;
			b=option[i][1];
    //if ((i< total1)&&(b==1)) b=10;
	//if ((i< total1+molecules[0][1]*molecules[0][2])&&(b==2)) b=22;
	//if ((i< total1+molecules[0][1]*molecules[0][2])&&(b==3)) b=23;
	    	fprintf(fp,"%d %2d",a,b);
...
void write_inputrd (int option[ATOM][2], int tot, double arr[ATOM][6], double center[psize][3], int mask[ATOM]){
    int i, j, k, a, b, c;
...
/*                    if(ddr <= fixc*fixc ){
    		...
    		for (j = 0; j < 3;j++){
    			...
    			if (arr[i][j]+0.0005>=axis[j]) arr[i][j]-=axis[j];
    		}
