...
int  read_dump (char* file, int option[ATOM][2], double array[ATOM][11]);
void read_config();
void write_dump (char *file, int option[][2], double array[][11]);
...
/* Screen out Settings --------------------------------------------------*/
	...
	posv=malloc(sizeof(double)*ATOM*11);
...
// This part is for the withOH model 
...
int read_dump (char *file, int option[ATOM][2], double arr[ATOM][11]) {
	int i, j, k, a, b, c;
    double x,y,z,vx,vy,vz,fx,fy,fz,q,msk;
    FILE *fp;
    char buf[BUF];
    ...
    for (i = 0; i < a;i++){
        fgets( buf, BUF, fp );
        sscanf( buf, "%d%d%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf",
                     &c,&b,&x,&y,&z,&vx,&vy,&vz,&fx,&fy,&fz,&q,&msk);
        option[c-1][0]=b;
        option[c-1][1]=b;
        arr[c-1][0]=x-daxis[0];
        arr[c-1][1]=y-daxis[1];
...
void write_dump (char *file, int option[][2], double arr[][11]) {
    int i, j, k, a, b, c;
    FILE *fp;
    fp = fopen(file,"w");
