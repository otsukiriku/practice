...
#define psize 1 /*number of C particle*/
#define delta 2.0	

char file1[BUF]="withwaterlayer.dump";
//char file1[BUF]="trimmed_dump";
char file2[BUF]="unwrapped_pos";

int total,total1,total2;
int tstep,interval;
int unit[3] = {3,3,3};
double fixc = 2.0;
double rmax[3]={123.5,135.0,135.0};
double rmin[3]={85.0,85.0,85.0};
...
int molecules[3][2]={{3,18000},{2,1875},{416,375}};
...
void write_inputrd (int option[ATOM][2], int tot, double array[ATOM][6], double center[psize][3]);
...
//        srand(16);
	...
	//printf("debug point1 : complete read dump file\n");
...
//				printf("%d\n",a);
	...
	if( fp4==NULL ){
		if (pid==0) printf("Warnning : There is no center.txt \n");
		panum=1;
...
//				printf("%d %f %f %f \n",panum,paori[i][0],paori[i][1],paori[i][2]);
	...
	fclose(fp4);
	//printf("debug point2 : complete read center.txt\n");
	
	xx=axis2[0]*(rand()%1000+500)/1000.0;
	yy=axis2[1]*(rand()%1000+500)/1000.0;
	zz=axis2[2]*(rand()%1000+500)/1000.0;
...
// hantei
	...
	fp1 = fopen("temp.rd","r");
	//printf("debug point3 : open temp.rd\n");
	for (i = 0; i < total3;i++){
		fgets( buf1, BUF, fp1 );
    	sscanf( buf1, "%d%lf%lf%lf", &a,&x,&y,&z);
        atom3c[i][0]=a;
	...
	fclose(fp1);
	//printf("debug point4 : end reading temp.rd\n");
...
//					printf("%d %.3lf %.3lf %.3lf \n",atom0c[c][0],pos0v[c][0],pos0v[c][1],pos0v[c][2]);
	...
	if (pid==0) minimum(pos0v, 25.0);
    if (pid==0) write_center(center);
	if (pid==0) write_inputrd(atom0c,total1,pos0v,center);
	if (pid==0) write_check(atom0c,pos0v);
        if (pid==0) remove("temp.rd");
...
void write_check (int option[ATOM][2], double arr[ATOM][6]) {
	...
	a=0;
        int na=total1+1;
        int nb=na+molecules[0][0]*molecules[0][1];
        int nc=nb+molecules[1][0]*molecules[1][1];
        int nd=nc+molecules[2][0]*molecules[2][1];

    for (i = 0; i < total;i++){
//    	if (option[i][1]<10){
                if ((i+1 < na)&&(option[i][0]==1)) option[i][1]=11;
                if ((i+1 >= na)&&(i+1 < nb)&&(option[i][0]==2)) option[i][1]=22;
                if ((i+1 >= na)&&(i+1 < nb)&&(option[i][0]==3)) option[i][1]=23;
                if ((i+1 >= nb)&&(i+1 < nc)&&(option[i][0]==1)) option[i][1]=31;
                if ((i+1 >= nb)&&(i+1 < nc)&&(option[i][0]==2)) option[i][1]=32;
                if ((i+1 >= nb)&&(i+1 < nc)&&(option[i][0]==3)) option[i][1]=33;
    		a++;
